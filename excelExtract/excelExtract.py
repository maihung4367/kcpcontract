...
def exportFiles(loaict,fileID,loaiAccount):
	...
	if loaiAccount=="All" and loaict=="All": 
		...
		for f in listAccount:
			...
			ws["D5"]="Tp.HCM, Ngày"
			ws['E5']=str(datetime.now().date().strftime("%d/%m/%y"))
			...
			ws['A11']="Account"
			#CHANGE  FONT STYLES
			b4=ws["B4"];	e5=ws["D5"];	f5=ws['E5']
			a6=ws["A6"];	a8=ws['A8'];	a10=ws['A10']
			a11=ws['A11'];	b10=ws['B10'];	b11=ws['B11']
			b4.font=boldFont;				a6.font=boldFont; 		e5.font=italicFont;				
			f5.font=italicFont;				a8.font=normalFont;		a10.font=normalFont;	
			...
			#header
			headers=['Mechanics: get/discount',"Noi dung chuong trinh","Post start date","Post end date","Sum of Budget RIR"]
			...
			for row,data in enumerate(fileData,start=14):
				for col,colAlphabet in enumerate(["A","B","C","D","E"],start=1):
			...
			ws['A{}'.format(str(rirSumLine+3))]="Mong Quý Khách Hàng cùng hợp tác với Công ty để đảm bảo các chương trình thực thi hiệu quả"
			ws['A{}'.format(str(rirSumLine+4))]="trong thời gian tới. Nếu Quý Khách Hàng có bất kỳ vấn đề nào cần làm rõ, vui lòng cho KCV được biết để cùng trao đổi"
			...
			ws.print_area = 'A1:F{}'.format(str(rirSumLine+7))
			# Printer Settings
		...
		fileName="{}_{}.xlsx".format(loaiAccount,str(datetime.now().date()))
		ws.print_area = 'A1:F{}'.format(str(rirSumLine+7))
		# Printer Settings
		ws.page_setup.orientation = ws.ORIENTATION_LANDSCAPE
		ws.page_setup.paperSize = ws.PAPERSIZE_A4
		...
		#header
		headers=['Mechanics: get/discount',"Post start date","Post end date"]
		for header in range(0,len(headers)):
			_=ws.cell(column=header+1,row=13,value=headers[header])
			_.font=boldFont
			_.fill=fillCellStyle
		...
		sumRir=0
		for row,data in enumerate(fileData,start=14):
			for col,colAlphabet in enumerate(["A","B","C"],start=1):
				if headers[col-1]=='Mechanics: get/discount':
		...
		ws.page_setup.paperSize = ws.PAPERSIZE_A4
		wb.save(fileName)
		pdf=open(pdfConvert(fileName), "rb")
		os.remove(os.getcwdb().decode('utf-8') + "\\{}".format(fileName))
		pdffile=pdfFile()
